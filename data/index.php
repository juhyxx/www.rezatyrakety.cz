<?php
	...
	$result = mysql_query("SELECT * FROM view_koncerty  WHERE trojkazblues_flag=1 and datum > now() ORDER BY datum ASC");

	while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
	    array_push($output['result']['trizblues'], $row);
	}
	
	$result = mysql_query("SELECT * FROM view_koncerty  WHERE trojkazblues_flag=1 and datum < now() ORDER BY datum DESC");

	while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
	    array_push($output['result']['oldtrizblues'], $row);
	}
